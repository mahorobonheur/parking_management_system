...
export default function ParkingMap({ title = 'Live parking map', parkingLotId, pollMs = 8000 }) {
  ...
  if (!data?.spaces?.length) {
    return (
      <div className="rounded-2xl border border-white/10 bg-slate-800/40 p-6 text-center text-slate-500">
              ...
              {row.map((cell, ci) => {
                if (!cell) {
                  return (
                    <div
                      key={`e-${ri}-${ci}`}
                      className="flex h-14 min-w-[3.5rem] items-center justify-center rounded-lg border border-dashed border-white/5 bg-slate-900/30 text-[10px] text-slate-600"
                    >
                      —
                    </div>
                  )
